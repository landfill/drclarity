...
> [!IMPORTANT]
> ### ⚠️ 계획 문서 상태 및 최종 구현 명세 안내
> 본 문서는 최초 구현 계획서이며, 작업 진행 중(WP-6/WP-7) **일부 방침이 실행 중 폐기 및 원본 복원**되었습니다.
>
> 1. **폐기 및 복원된 사항**:
>    - **데스크톱 1뷰포트 게이트 폐기**: 수치 기준(scrollHeight/overflow/44px)은 전부 통과했으나 실제 화면에서 하단 큰 공백과 히어로 축소 등 디자인 품질이 나빠졌고, 사용자가 원본 비율 복원을 결정함.
>    - **강제 2컬럼 폐기**: `TopicLayout.wide` / `SolutionStepper.split` 을 이용한 강제 2컬럼 호출부를 제거하고 원본 단일 컬럼으로 복원 (`wide` / `split` prop 은 컴포넌트 API 로 남아 있으나 현재 어느 페이지도 사용하지 않음).
>    - **토큰 축소값 복원**: 여백·타이포 축소 토큰 값을 원본 스케일로 복원함. **최종 토큰 값의 단일 원천(SSOT)은 `src/app/globals.css` 이며, 본 문서의 §2 토큰 블록은 최초 계획값입니다.**
>    - **honey-pots 문제 이미지 보존**: `problem.png` 정적 문제 삽화 렌더 블록 복원 유지.
>
> 2. **현재 최종 유효한 사항**:
>    - 토큰 레이어 구조 및 `--header-h` 단일 원천화
>    - `body { overflow-x: hidden }` 제거 및 전 라우트 가로 오버플로 해소
>    - 44×44px 터치타깃 규격 준수
>    - 힌트 UI 일원화 (`TopicLayout.hint` 와 `SolutionStepper.showHintInline` 은 최종적으로 **제거됨**)
>    - 중복 콘텐츠 정리 및 `CalculatorReveal` / `SolutionStepper` 의 조건부 자동 스크롤
>
> 3. **문서 열람 주의**:
>    본 문서의 하위 WP별 상세 내역은 실행 이력으로서 보존된 것이며, 최종 구현 상태 판단 시 위 사항 및 실제 코드(`globals.css` 등)를 우선합니다.
...
> ※ **안내**: 아래 토큰 블록은 작업 초기 **최초 계획값**입니다. 이후 원본 비율 복원(WP-6/WP-7)을 거친 **최종 토큰 값은 `src/app/globals.css`를 참조**하세요.
...
- [ ] 문제 이미지가 데스크톱 240px / 모바일 180px 높이를 넘지 않고 비율이 유지된다 (실측 첨부) *(※ 최초 계획값. 최종 미디어 상한값 400px/260px 및 이미지 보존 방침은 globals.css 및 상단 안내 참조)*
