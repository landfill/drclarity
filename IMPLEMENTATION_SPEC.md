...
산문을 손으로 옮기면 문장이 빠지거나 미묘하게 달라질 수 있다. **빌드도 테스트도 이것을 잡지 못하고**, 브라우저로 봐도 "정상 렌더" 는 알 수 있어도 "원문과 같은가" 는 알 수 없다.

```bash
node scripts/check-content-drift.mjs <이관 직전 커밋>
```

삭제된 TSX 텍스트와 추가된 MDX 텍스트를 정규화해 대조한다. 보고되는 조각 대부분은 제목이 `title` export 로 갈리면서 끊긴 **경계**이고, 스크립트가 이를 판정해 걸러 낸다. 남는 것만 실제 손실 후보다.
