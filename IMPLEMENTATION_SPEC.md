...
├── package.json
├── scripts/
│   └── generate-topic-registry.mts        # §4 콘텐츠 자동 수집기
├── public/
│   └── topics/
│       ├── honey-pots/
...
### 4.3 수집기 — `scripts/generate-topic-registry.mts` (MUST)
...
```ts
// AUTO-GENERATED by scripts/generate-topic-registry.mts — DO NOT EDIT.
...
{
  "scripts": {
    "generate:registry": "node scripts/generate-topic-registry.mts",
...
- Node 22.18 은 타입 스트리핑이 기본 활성화되어 있어 `.mts` 를 직접 실행할 수 있다. 실행에 실패하면 순서대로 시도한다: `node --experimental-strip-types …` → 확장자를 `.mjs` 로 바꾸고 타입 주석 제거 (MAY).
...
- `scripts/generate-topic-registry.mts` + 검증 로직 (§4.3)
