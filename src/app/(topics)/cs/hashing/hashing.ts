 ...
 * 두 해시가 몇 비트나 다른가.
 *
 * 이 주제의 숫자다. 입력을 한 글자만 고쳐도 이 값이 256의 절반 근처에서 나오고, 완전히
 * 다른 두 문장을 넣어도 마찬가지다 — "조금 바꾸면 조금 바뀐다" 는 직관이 여기서 깨진다.
 * 입력이 얼마나 비슷했든 나오는 숫자가 비슷하므로, 해시의 비트 패턴만으로는 두 입력의
 * 유사도를 가늠할 수 없다. (후보 목록을 들고 대입하는 것은 별개 이야기다.)
 */
...
/**
 * 공격자가 미리 계산해 두는 표의 축소판.
 *
 * 흔히 '레인보우 테이블' 이라고 부르지만 엄밀히는 다르다. 진짜 레인보우 테이블은 해시와
 * 축약을 번갈아 이어 붙인 사슬의 양 끝만 저장해 자리를 아끼는 구조다. 여기 있는 것은
 * 후보마다 해시를 통째로 들고 찾아보는 단순한 사전이라, 화면에도 그렇게만 적었다.
 *
