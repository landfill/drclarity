 ...
 * 시나리오 대화.
 *
 * **첫 메시지에 이름이 들어 있다.** 이 주제의 장치 전부가 거기 걸려 있다 — 대화가
 * 길어지면 이 메시지가 가장 먼저 창 밖으로 밀려나고, 그 순간 모델은 이름을 답할 수 없게 된다.
 */
export const SCRIPT: Turn[] = [
  { id: 't1', role: 'user', text: '안녕하세요! 저는 지민이라고 합니다. 다음 주에 부산으로 여행을 가요.' },
  { id: 't2', role: 'ai', text: '안녕하세요 지민님. 부산 여행 좋네요! 무엇을 도와드릴까요?' },
 ...
 * 화면에 놓을 창 크기.
 *
 * 시나리오 전체가 677토큰이다. 가장 넓은 창은 그보다 커서 **아무것도 밀려나지 않는
